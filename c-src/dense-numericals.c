...
#define two_arg_fn_body(taf)                                              \
  ...
  void DN_d##taf(const long n,                                            \
    ...
    vecf64 va, vb, vc;                                                    \
    if (incx == 1 && incy == 1 && inc_out == 1){                          \
      double* simd_end = out + (n/SIMD_SINGLE_STRIDE)*SIMD_SINGLE_STRIDE;  \
      while(out != simd_end){                                             \
        va = vecf64_load(x);                                              \
        vb = vecf64_load(y);                                              \
      ...
      double* simd_end = x + (n/SIMD_SINGLE_STRIDE)*SIMD_SINGLE_STRIDE;  \
      while(x != simd_end){                                               \
        va = vecf64_load(x);                                              \
        vb = vecf64_load(y);                                              \
        vc = sleefify_vecf64(taf)(va, vb);                                \
      ...
      double* simd_end = y + (n/SIMD_SINGLE_STRIDE)*SIMD_SINGLE_STRIDE;  \
      while(y != simd_end){                                               \
        va = vecf64_make(x, incx);                                         \
        vb = vecf64_load(y);                                              \
        vc = sleefify_vecf64(taf)(va, vb);                                \
